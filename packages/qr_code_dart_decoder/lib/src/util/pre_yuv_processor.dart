...
abstract class PreYuvProcessor {
  List<Yuv420Planes>? process(List<Yuv420Planes> image) {
    return image;
  }
}

class CropBackgroundYuvProcessor extends PreYuvProcessor {
  final bool debug;
  ...
  List<Yuv420Planes>? process(List<Yuv420Planes> image) {
    ...
    if (debug) {
      ...
      if (rect != null) {
        ...
        print(
            'crop applied: left=${rect.left}, top=${rect.top}, width=${rect.width}, height=${rect.height}');

        // Save debug image if possible
        try {
          ...
          final resultHeight =
              resultPlane.height ?? (resultPlane.bytes.length / resultWidth).round();
          ...
          print('Failed to save processed YUV image: $e');
        }
      } else {
